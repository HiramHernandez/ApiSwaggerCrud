...
class DeviceService:
    ...
    def fetch(self):
        try:
            return  (db.session.query(Device).all(), False)
        except  Exception as e:
            self._logger.error(f"Error al obtener dispositivos: {e}", exc_info=True)  # Registra el error con traceback
            return  ([], True )
    ...
    def retrieve(self, id):
        try:
           return (db.session.query(Device).filter(Device.id == id).first(), False)   
        except Exception as e:
            self._logger.error(f"Error al buscar dispositivo: {e}", exc_info=True)
            return (None, True)
    ...
    def create(self, data):
        try:
            ...
            self._logger.error(f"Error al crear el dispositivo: {e}", exc_info=True)
            return (None, True)
    ...
    def update(self, device_id, data):
        try:
            ...
            device.fecha_actualizacion = datetime.now()
            
            db.session.commit()
            return (device, True)
        except Exception as e:
            self._logger.error(f"Error al actualizar el dispositivo: {e}", exc_info=True)
            return (None, False)
